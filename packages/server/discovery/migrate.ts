...
export async function migrateDiscoveryStorage(pool:Pool,path:string):Promise<void>{
 ...
 try{await client.query('BEGIN');await client.query('SELECT pg_advisory_xact_lock(741917,1)');
  const prior=(await client.query('SELECT digest FROM app_migrations WHERE id=$1',['005_discovery_receipts'])).rows[0];
  if(prior){if(prior.digest!==digest)throw new Error('Applied migration checksum differs');}
  else{await client.query(sql);await client.query('INSERT INTO app_migrations(id,digest) VALUES($1,$2)',['005_discovery_receipts',digest]);}
