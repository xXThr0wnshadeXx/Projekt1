...
export interface ContactsStore {
  ...
  /** Recheck private scope ownership, atomically enable/provision the bound Google source and store credentials.
   * Preserve one source per (ownerUserId, scopeId, googleSubject). No graph relationships are inferred here.
   */
  commitContactsGrant(grant: ContactsGrant): Promise<void>;
