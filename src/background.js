...
function queueUnexplored(s,minDepth=0){
  ...
  for(const person of Object.values(s.nodes).sort((a,b)=>a.depth-b.depth||a.id.localeCompare(b.id))){
    ...
    if(!s.branches[person.id]&&!busy.has(person.id)){
      s.queue.push({kind:'profile',owner:person.id,depth:person.depth});busy.add(person.id);added++;continue;
    }
    ...
    if(repairs>=24||busy.has(person.id))continue;
    const checked=s.profileChecks?.[person.id];
    if(!person.location&&!checked){
      s.queue.push({kind:'profile',owner:person.id,depth:person.depth,detailsOnly:true});busy.add(person.id);repairs++;added++;continue;
    }
    ...
    if(!comments||!comments.discoveryVersion&&['hidden','incomplete'].includes(comments.status)||comments.status==='queued'){
      ...
      if(s.queue.length>before){busy.add(person.id);repairs++;added++;}
    }
  }
  return added;
}
...
async function command(message){
  ...
  if(message.type==='RESUME'){
    ...
    for(const w of workers(s)){
      ...
      const missingPosts=!comments||comments.status==='queued'||!comments.discoveryVersion&&['hidden','incomplete'].includes(comments.status);
