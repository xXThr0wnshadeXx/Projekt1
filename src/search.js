...
export function scorePerson(person,query){
  ...
  for(const term of expansions){if(name===term){score=Math.max(score,100);reason='Exact name';}else if(name.startsWith(term)){score=Math.max(score,92);reason='Name starts with this';}else if(new RegExp(`(?:^| )${term.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?: |$)`).test(name)){score=Math.max(score,88);reason='Name match';}else if(name.includes(term)){score=Math.max(score,82);reason='Name contains this';}else if(doc.includes(term)){score=Math.max(score,72);reason='Profile detail match';}}
  const queryTokens=words(q).filter(token=>token.length>=3&&!stopWords.has(token));if(queryTokens.length){let matched=0,total=0,exact=0;for(const queryToken of queryTokens){let best=0;for(const token of tokens)best=Math.max(best,tokenSimilarity(queryToken,token));if(best>=.62){matched++;total+=best;if(best===1)exact++;}}const coverage=matched/queryTokens.length;if(matched&&(queryTokens.length===1||coverage>=.6)){const fuzzy=Math.round(42+coverage*24+(total/matched)*18);if(fuzzy>score){score=fuzzy;reason=exact===queryTokens.length?'Related keyword match':coverage===1?'Close spelling or term match':'Related profile terms';}}}
  return {score,reason};
}
