import {profileURL,options,route,exportGraph,importGraph,csv} from './core.js';
...
const statuses={running:'Collecting',paused:'Paused',limit:'Person limit reached',complete:'Queue finished',imported:'Imported map'};
...
async function send(message){const response=EXTENSION?await chrome.runtime.sendMessage(message):await chrome.runtime.sendMessage(COMPANION_ID,message);if(!response?.ok)throw Error(response?.error||'The collector did not respond. Reload the extension and try again.');return response;}
...
function download(name,content,type){const blob=new Blob([content],{type}),url=URL.createObjectURL(blob),a=el('a');a.href=url;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
function fileStem(){return `orbit-${state?.root?.split('/')[4]||'network'}-${new Date().toISOString().slice(0,10)}`;}
function filteredPeople(){const q=$('search').value.toLowerCase().trim();return Object.values(state?.nodes||{}).filter(p=>`${p.name} ${p.headline} ${p.location}`.toLowerCase().includes(q)).sort((a,b)=>a.depth-b.depth||a.name.localeCompare(b.name));}
...
function renderInspector(){const p=state?.nodes[selected];if(!p){graph.focus(null);return;}const target=$('inspector-content');target.replaceChildren();target.append(el('div',p.name.split(/\s+/).slice(0,2).map(w=>w[0]).join('').toUpperCase(),'person-avatar'),el('h3',p.name));if(p.headline)target.append(el('p',p.headline));if(p.location)target.append(el('p',p.location));target.append(link('View LinkedIn profile ↗',p.url,'profile-link'));const path=route(state,p.id);graph.focus(p.id,path);const section=el('div',undefined,'detail-section');section.append(el('h4','SHORTEST RECORDED PATH'));if(path.length)for(const id of path)section.append(el('div',state.nodes[id].name,'path-person'));else section.append(el('p','No recorded path to the starting profile.'));target.append(section);
  const edges=Object.values(state.edges).filter(e=>e.source===p.id||e.target===p.id);if(edges.length){const neighbors=el('div',undefined,'detail-section');neighbors.style.marginTop='24px';neighbors.append(el('h4',`${edges.length} CONNECTIONS IN THIS MAP`));for(const edge of edges.slice(0,20)){const other=edge.source===p.id?edge.target:edge.source,b=el('button',state.nodes[other].name,'neighbor');b.onclick=()=>selectPerson(other);neighbors.append(b);}if(edges.length>20)neighbors.append(el('p',`${edges.length-20} more connections appear in the graph and exports.`));target.append(neighbors);const sources=el('div',undefined,'detail-section');sources.style.marginTop='24px';sources.append(el('h4','RELATIONSHIP EVIDENCE'));for(const edge of edges.slice(0,5)){const other=edge.source===p.id?edge.target:edge.source,e=edge.evidence[0];if(e)sources.append(link(`${state.nodes[other].name} · source list ↗`,e.url,'evidence-link'));}target.append(sources);}}
function renderLive(){
...
function render(){
  renderLive();
  const nodes=Object.values(state?.nodes||{}),edges=Object.values(state?.edges||{}),branches=Object.values(state?.branches||{});set('people-count',state?nodes.length.toLocaleString():'—');set('edge-count',state?edges.length.toLocaleString():'—');set('branch-count',state?branches.filter(b=>b.pages>0).length.toLocaleString():'—');set('page-count',state?(state.pages||0).toLocaleString():'—');set('network-title',state?`${state.nodes[state.root]?.name||'Starting profile'}’s network`:'Your connection map');set('run-badge',statuses[state?.status]||'Not started');$('run-badge').className=`badge ${state?.status||''}`;set('status-label',statuses[state?.status]||'Ready to explore');$('status-dot').className=state?.status||'';set('status-reason',state?.reason||'Collected discoveries save to your private library while this page is open.');show('empty-graph',!state);show('collector-controls',hasCollector()&&Boolean(state)&&state.status!=='imported');show('pause',state?.status==='running');show('resume',['paused','limit'].includes(state?.status));show('show-tab',Boolean(state?.tabId));show('clear-button',!libraryMode&&Boolean(state)&&state.status!=='running');$('start').disabled=libraryMode||Boolean(state)&&['running','paused','limit'].includes(state.status);$('export-button').disabled=!state;$('csv-button').disabled=!state;$('edges-csv-button').disabled=!state;$('import-button').disabled=libraryMode||Boolean(state)&&['running','paused','limit'].includes(state.status);$('profile-url').disabled=Boolean(state)&&['running','paused','limit'].includes(state.status);set('updated-label',state?`${state.cloudView?'Private library':'Collection checkpoint'} · ${new Date(state.updatedAt).toLocaleString()}`:'Private permanent library');set('visible-label',state?`${nodes.filter(p=>p.depth>1).length.toLocaleString()} people beyond the first layer`:'Up to 10,000 people per map');graph.setData(state);graph.search($('search').value);if(view==='directory')renderPeople();if(view==='coverage')renderCoverage();renderInspector();
}
...
$('clear-button').onclick=async()=>{if(!confirm('Clear the saved network from this browser? Export it first if you want to keep a copy.'))return;try{if(hasCollector())await send({type:'CLEAR'});else localStorage.removeItem(KEY);selected=null;$('inspector-content').replaceChildren(el('h3','Every person has a path.'),el('p','Select someone in your map to explore their connections.'));await refresh();}catch(e){toast(e.message);}};
$('export-button').onclick=()=>{if(state)download(`${fileStem()}.network.json`,JSON.stringify(exportGraph(state),null,2),'application/json');};
$('csv-button').onclick=()=>{if(state)download(`${fileStem()}-people.csv`,csv([['Name','LinkedIn URL','Headline','Location','Distance from root'],...Object.values(state.nodes).map(p=>[p.name,p.url,p.headline,p.location,p.depth])]),'text/csv;charset=utf-8');};
$('edges-csv-button').onclick=()=>{if(state)download(`${fileStem()}-connections.csv`,csv([['Person A URL','Person B URL','Source URL','Observed at'],...Object.values(state.edges).flatMap(e=>e.evidence.map(v=>[e.source,e.target,v.url,v.observedAt]))]),'text/csv;charset=utf-8');};
...
$('paste-import').onclick=async()=>{try{const text=$('import-json').value;if(text.length>50_000_000)throw Error('Choose a network smaller than 50 MB.');const data=JSON.parse(text),imported=importGraph(data);if(hasCollector())await send({type:'IMPORT',data});else localStorage.setItem(KEY,JSON.stringify(imported));$('import-dialog').close();$('import-json').value='';await refresh();graph.fit();toast('Network imported.');}catch(e){toast(e.message);}};
$('import-file').onchange=async()=>{try{const file=$('import-file').files[0];if(!file)return;if(file.size>50_000_000)throw Error('Choose a network file smaller than 50 MB.');const data=JSON.parse(await file.text()),imported=importGraph(data);if(hasCollector())await send({type:'IMPORT',data});else localStorage.setItem(KEY,JSON.stringify(imported));$('import-dialog').close();await refresh();graph.fit();toast('Network imported.');}catch(e){toast(e.message);}finally{$('import-file').value='';}};
for(const name of ['graph','directory','coverage'])$(`tab-${name}`).onclick=()=>switchView(name);
$('search').oninput=()=>{graph.search($('search').value);if(view==='directory')renderPeople();};$('fit').onclick=()=>graph.fit();$('zoom-in').onclick=()=>graph.zoom(1.3);$('zoom-out').onclick=()=>graph.zoom(1/1.3);
