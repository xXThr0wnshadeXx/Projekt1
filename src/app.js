...
let filterTimer=null,searchTimer=null,filterOptionsRevision=-1,directoryRevision=-1,directoryPeople=[];
...
$('search').oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(applySearch,120);};$('fit').onclick=()=>graph.fit();$('zoom-in').onclick=()=>graph.stepZoom(1);$('zoom-out').onclick=()=>graph.stepZoom(-1);
  ...
  async function refreshShared(){const root=collectionState?.root||globalThis.ORBIT_PROFILE;if(sharedRefreshing||libraryMode||!root||document.visibilityState!=='visible')return;sharedRefreshing=true;try{await library.loadAccount(root,6);await syncSharedToCollector();}finally{sharedRefreshing=false;}}
...
function refreshFilterOptions(){
  ...
  const filters=graph.filters,visible=people.filter(p=>graph.matchesFacet(p)),active=[filters.location&&`location “${filters.location}”`,filters.field&&`sector “${filters.field}”`,filters.keywords?.length&&`profile “${filters.keywords.join(', ')}”`].filter(Boolean);
