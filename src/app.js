...
const library=createLibrary({getCollection:()=>collectionState,showGraph:(s,accountView=false)=>{libraryMode=!accountView;if(accountView){sharedState=s;syncSharedToCollector(s).catch(()=>{});}state=accountView?accountGraph():s;selected=null;render();},showCollection:()=>{libraryMode=false;state=accountGraph();selected=null;render();}});
...
$('search').oninput=()=>{clearTimeout(searchTimer);searchTimer=setTimeout(applySearch,120);};$('fit').onclick=()=>graph.fit();$('zoom-in').onclick=()=>graph.stepZoom(1);$('zoom-out').onclick=()=>graph.stepZoom(-1);
  ...
  async function refreshShared(){const root=collectionState?.root||globalThis.ORBIT_PROFILE;if(sharedRefreshing||libraryMode||!root)return;sharedRefreshing=true;try{await library.loadAccount(root,6);}finally{sharedRefreshing=false;}}
