...
const statuses={running:'Collecting',paused:'Paused · progress saved',limit:'Person limit reached',cancelled:'Saved · queue cleared',complete:'Up to date',imported:'Saved map'};
...
function render(){
  ...
  const nodes=Object.values(state?.nodes||{}),complete=state?.status==='complete',paused=['paused','limit'].includes(state?.status);renderDatabaseMetrics();set('network-title',state?`${state.nodes[state.root]?.name||'Starting profile'}’s network`:'Your account network');set('run-badge',state?statusLabel(state):'Not started');$('run-badge').className=`badge ${state?.status||''}`;set('status-label',statusLabel(state));$('status-dot').className=state?.status||'';set('status-reason',state?.reason||'Discoveries periodically save to your account and merge safely into the team graph.');set('pause-reason',paused?(state.reason||'Collection paused. Open the collection tab, then resume.'):'');show('pause-reason',paused);const actionLabel=complete?'Check for updates ↗':paused?'Resume collection ↗':state?.status==='running'?'Adjust collection':'Start collecting ↗',settingsAction=complete?'Apply and check again ↗':paused?'Apply and resume ↗':state?.status==='running'?'Apply changes ↗':'Start collecting ↗';set('workspace-build',actionLabel);set('start',settingsAction);show('empty-graph',!state);show('collector-controls',hasCollector()&&Boolean(state)&&state.status!=='imported');show('pause',state?.status==='running');show('resume',paused);show('show-tab',Boolean(state?.tabId));show('clear-button',!libraryMode&&Boolean(state)&&state.status!=='running');$('workspace-build').disabled=libraryMode;$('start').disabled=libraryMode;$('profile-url').disabled=Boolean(collectionState);set('updated-label',state?`${state.sharedView?'Database-first account map':state.cloudView?'Account network rebuilt from D1':'Account checkpoint autosaved'} · ${new Date(state.updatedAt).toLocaleString()}`:'Shared team library');set('visible-label',state?`${nodes.filter(p=>p.depth===1).length.toLocaleString()} direct · ${nodes.filter(p=>p.depth>1).length.toLocaleString()} connected through shared paths`:'One persistent account network');graph.setData(state);refreshFilterOptions();if(view==='directory')renderPeople();if(view==='coverage')renderCoverage();
}
...
$('pause').onclick=async()=>{try{await send({type:'PAUSE'});await refresh();toast('Collection paused. The exact page, pending queue, people, and relationships are saved.');}catch(e){toast(e.message);}};
...
function openWorkspaceSettings(settings){
 $('map-settings').hidden=!settings;$('settings-scrim').hidden=!settings;document.body.classList.toggle('settings-open',settings);if(settings)$('inspector').close();
...
$('close-settings').onclick=$('settings-scrim').onclick=()=>openWorkspaceSettings(false);
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('map-settings').hidden)openWorkspaceSettings(false);});

$('close-person').onclick=()=>$('inspector').close();
$('workspace-build').onclick=()=>{
 if(state?.status==='running'){openWorkspaceSettings(true);return;}
