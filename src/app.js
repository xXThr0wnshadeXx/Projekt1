...
function renderInspector(){const p=state?.nodes[selected];if(!p){graph.focus(null);return;}const serial=++inspectorSerial,target=$('inspector-content');target.replaceChildren();target.append(el('div',p.name.split(/\s+/).slice(0,2).map(w=>w[0]).join('').toUpperCase(),'person-avatar'),el('h3',p.name));if(p.headline)target.append(el('p',p.headline));if(p.location)target.append(el('p',p.location));const actions=el('div',undefined,'inspector-actions'),share=el('button','Share person & route','quiet');actions.append(link('View LinkedIn ↗',p.url,'profile-link'),share);share.onclick=async()=>{const path=route(state,p.id).map(id=>state.nodes[id]?.name).filter(Boolean).join(' → '),text=[p.name,p.headline,p.location,path&&`Observed route: ${path}`,p.url].filter(Boolean).join('\n');try{if(navigator.share)await navigator.share({title:`${p.name} · Orbit`,text});else{await navigator.clipboard.writeText(text);toast('Person and observed route copied.');}}catch(error){if(error.name!=='AbortError')toast('Sharing was not available in this browser.');}};target.append(actions);
  ...
  const localPath=route(state,p.id);graph.focus(p.id,localPath);const section=el('div',undefined,'detail-section');section.append(el('h4','BEST ROUTE IN THIS VIEW'));if(localPath.length)for(const id of localPath)section.append(el('div',state.nodes[id].name,'path-person'));else section.append(el('p','No route appears in the currently loaded view.'));target.append(section);
...
function render(){
  ...
  const nodes=Object.values(state?.nodes||{}),edges=Object.values(state?.edges||{}),branches=Object.values(state?.branches||{});set('people-count',state?nodes.length.toLocaleString():'—');set('edge-count',state?edges.length.toLocaleString():'—');set('branch-count',state?branches.filter(b=>b.pages>0).length.toLocaleString():'—');set('page-count',state?(state.pages||0).toLocaleString():'—');set('network-title',state?`${state.nodes[state.root]?.name||'Starting profile'}’s account network`:'Your account network');set('run-badge',statuses[state?.status]||'Not started');$('run-badge').className=`badge ${state?.status||''}`;set('status-label',statuses[state?.status]||'Ready to explore');$('status-dot').className=state?.status||'';set('status-reason',state?.reason||'Discoveries periodically save to your account and merge safely into the team graph.');show('empty-graph',!state);show('collector-controls',hasCollector()&&Boolean(state)&&state.status!=='imported');show('pause',state?.status==='running');show('resume',['paused','limit'].includes(state?.status));show('show-tab',Boolean(state?.tabId));show('clear-button',!libraryMode&&Boolean(state)&&state.status!=='running');$('workspace-build').disabled=libraryMode||Boolean(state)&&['running','paused','limit'].includes(state.status);$('start').disabled=libraryMode||Boolean(state)&&['running','paused','limit'].includes(state.status);$('profile-url').disabled=Boolean(state)&&['running','paused','limit'].includes(state.status);set('updated-label',state?`${state.cloudView?'Account network loaded from D1':'Account checkpoint autosaved'} · ${new Date(state.updatedAt).toLocaleString()}`:'Shared team library');set('visible-label',state?`${nodes.filter(p=>p.depth===1).length.toLocaleString()} direct · ${nodes.filter(p=>p.depth>1).length.toLocaleString()} extended`:'One persistent account network');graph.setData(state);refreshFilterOptions();graph.search($('search').value);if(view==='directory')renderPeople();if(view==='coverage')renderCoverage();
}
