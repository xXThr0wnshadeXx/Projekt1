...
export function nextAction(policy, delay, now = Date.now()) {
...
export function reserveAction(policy, delay, now = Date.now()) {
  const p = normalizePolicy(policy, now), gate = nextAction(p, delay, now);
