import {matchesFilters,groupTargets,springProgress,locationOf,fieldOf,keywordGroupOf,keywordTerms} from './filters.js';
export class NetworkGraph {
  ...
  setFilters(filters,by='none',keywords=[]){this.filters=filters;this.groupBy=by;this.keywords=keywordTerms(keywords);this.layout();this.draw();}
  layout(){
    const now=performance.now();this.advanceMotion(now);
    const grouped=this.groupBy==='none'?null:groupTargets(this.points.filter(p=>this.isVisible(p)),this.groupBy,this.keywords);
