...
test('session uses trusted identity, works without a database, and is never cached',async()=>{
  ...
  assert.deepEqual(await signedIn.json(),{authenticated:true,id:'chatgpt:alice',email:'alice@example.com',displayName:null,provider:'chatgpt',linkedinProfileUrl:null,onboardingComplete:false,googleEnabled:false});
});
