...
test('combined application migrates, confirms facts over HTTP and returns qualified search paths',{skip:!url},async()=>{
 ...
 async function owner({relationships=false}={}){
  ...
  const ledger=await admin.query(`SELECT id FROM ${schema}.app_migrations ORDER BY id`);assert.deepEqual(ledger.rows.map(r=>r.id),['001_private_storage','002_contacts_grants','003_fact_reviews','004_public_fact_staging','005_discovery_receipts','007_discovery_staging']);
