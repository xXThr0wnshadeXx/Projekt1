...
test('a lost first-degree post job is recovered independently of its completed connection list',async t=>{
  const s=newState(root,{depth:2,comments:true});s.status='complete';s.queue=[];s.nodes[a]={...person('a'),id:a,depth:1,location:'Example City'};s.branches[root]={status:'exhausted',pages:1,profiles:[a]};s.branches[a]={status:'exhausted',pages:2,profiles:[]};completeDetails(s);
  ...
  await h.command({type:'START',url:root,config:{depth:2,comments:true}});assert.equal(h.data.orbitNetwork.current.job.kind,'posts');assert.equal(h.data.orbitNetwork.current.job.owner,a);
...
test('coverage counts inspected posts even when no commenter is readable',async t=>{
  const h=await harness(t,newState(root,{depth:2,comments:true})),activity=root+'recent-activity/all/';h.snapshots={[root]:{...profile(person('root'),null),activityUrl:activity},[activity]:{kind:'posts',url:activity,owner:root,cards:[{urn:'urn:li:activity:123',author:root,post:'https://www.linkedin.com/feed/update/urn:li:activity:123/',comments:[],control:null}]}};
...
test('updating during a long first-degree list resumes skipped posts before replaying its saved page',async t=>{
  const s=newState(root,{depth:2,comments:true});s.status='paused';s.queue=[];s.nodes[a]={...person('a'),id:a,depth:1,location:'Example City'};s.branches[root]={status:'exhausted',pages:1,profiles:[a]};s.branches[a]={status:'collecting',pages:4,profiles:[],seenPages:['saved'],url:list('a')};completeDetails(s);
