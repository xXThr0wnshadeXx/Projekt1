...
describe('real PostgreSQL Contacts consent and encrypted grant storage', {skip: !url}, () => {
  ...
  async function owner() {
    ...
    await store.putSession({tokenHash: sessionHash, userId: user.userId, createdAt: 100, expiresAt: 1000, revokedAt: null});
  ...
  it('atomically creates one authorized source and evidenced root identity without adding contacts or relationships', async () => {
    const a = await owner();
    await Promise.all(Array.from({length: 4}, () => store.commitContactsGrant(a.grant)));
    ...
    await rejectsCode(() => store.commitContactsGrant({...a.grant, sourceId: randomUUID()}), 'VERSION_CONFLICT');
  });
  it('rejects missing Contacts permission and cross-owner grant access/update', async () => {
    const a = await owner(), b = await owner();
    await rejectsCode(() => store.commitContactsGrant({...a.grant, grantedScopes: ['openid']}), 'INVALID_INPUT');
    await rejectsCode(() => store.commitContactsGrant({...a.grant, scopeId: b.scopeId}), 'FORBIDDEN');
    await rejectsCode(() => store.commitContactsGrant({...a.grant, googleSubject: b.user.googleSubject}), 'FORBIDDEN');
    await store.commitContactsGrant(a.grant);
  ...
  it('rolls source, root identity and graph version back if encrypted-grant persistence fails', async () => {
    ...
    try { await assert.rejects(() => store.commitContactsGrant(a.grant)); }
  ...
  it('allows one refresh winner, rejects stale refresh after consent, and erases revoked credentials while retaining evidence', async () => {
    const a = await owner(); await store.commitContactsGrant(a.grant);
    ...
    const consent = {...refresh, updatedAt: 400, version: randomUUID()}; await store.commitContactsGrant(consent);
  ...
  it('rechecks enabled source and current policy on every credential read/refresh', async () => {
    ...
    assert.equal(await store.getContactsGrant(a.user.userId, a.grant.sourceId), null);
    assert.equal(await store.replaceContactsGrant({...a.grant, version: randomUUID()}, a.grant.version), false);
    await rejectsCode(() => store.commitContactsGrant({...a.grant, version: randomUUID()}), 'VERSION_CONFLICT');
  });
});
