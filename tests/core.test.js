...
test('a database graph renders as the account map even without a local collector checkpoint',()=>{const shared={root,nodes:{[root]:{id:root,name:'Root',depth:0}},edges:{},coverage:[],cloudView:true};assert.equal(mergeAccountGraphs(null,shared),shared);});
