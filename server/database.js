...
export function validateBatch(data){
  if(!Array.isArray(data.nodes)||!Array.isArray(data.edges)||data.nodes.length>MAX_BATCH||data.edges.length>MAX_BATCH)throw Error('Save at most 100 people and 100 links per batch.');
  const now=new Date().toISOString();
  const nodes=data.nodes.map(p=>{const id=profileURL(p.id||p.url);if(!id)throw Error('Invalid profile URL.');const name=short(p.name,200);return {id,name,search_name:name.toLowerCase(),headline:short(p.headline,1000),location:short(p.location,300),at:now};});
...
export async function ingest(db,owner,data){
  const {nodes,edges}=validateBatch(data),nodeJSON=JSON.stringify(nodes),edgeJSON=JSON.stringify(edges),observations=JSON.stringify(edges.flatMap(e=>e.observations.map(o=>({a:e.a,b:e.b,...o}))));
...
export async function stats(db,owner){
  const r=await db.prepare('SELECT (SELECT COUNT(*) FROM people WHERE owner=?) people,(SELECT COUNT(*) FROM connections WHERE owner=?) connections,(SELECT MAX(last_seen) FROM people WHERE owner=?) lastSaved').bind(owner,owner,owner).first();return r;
}
