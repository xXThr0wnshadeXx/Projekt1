...
- **Companion download:** [download the current ZIP](https://orbit-shreev2703-graph-test.shreev2703.chatgpt.site/downloads/orbit-network-mapper.zip?v=2.8.6)
- **Current application and companion version:** `2.8.6`
...
Each eligible person’s posts are checked before crawling their connection list. At the same depth, queued posts and profile checks take priority over list pagination, so a large connection list cannot keep other first-degree profiles from reaching their posts. Profile details, connection lists, and post coverage are tracked independently: Resume, Explore next layer, and continuing a saved map recover a bounded batch of missing locations and skipped post jobs without replaying completed lists. A metadata check is saved even when LinkedIn does not show a location, preventing repeated repair requests. Coverage counts every inspected original post, including posts with no readable commenters.
