...
## Reviewed manual-facts HTTP and migration composition

Integrated independently cleared `1790b6221bd3412cfc95101d1f00abc358a68da2` as `053672a`, preserving owner modules. Real startup storage now awaits migrations001,002,003 in order;003 uses the reviewed checksum ledger and shared advisory transaction lock. The same pool supplies `PgFactStore`, and application composition constructs `FactReviewService` with the real session adapter. The existing search engine is decorated with `withFactWarnings`, preserving ranking while qualifying manual confidence, recency and unknown willingness in results and candidate paths.

HTTP contract:

- `GET /api/facts/review?scopeId=...` returns the facade's safe current relationships/affiliations, exact affiliation keys, sources/evidence and warnings.
- Same-origin `POST /api/facts/confirm` accepts only `{scopeId,expectedGraphVersion,idempotencyKey,confirm:true,change}` under the strict reviewed FactChange union and returns the committed decision/version/events receipt. One explicit acceptance or rejection per request; independent includeInSearch/currentness choices remain required by the module.

Both routes require a valid session and pass the extracted opaque credential to the facade, which derives the actor/session hash internally. Session/scope/source/transaction checks remain in the reviewed module. Existing16KiB bounded JSON, same-origin protection, no-store and sanitized error mapping apply; oversized bodies return413. Missing facts storage returns SOURCE_UNAVAILABLE after authentication/validation. No new successful discovery response was invented: /api/discovery and /api/discovery/capabilities are reserved for the next reviewed module and remain404.

Verification: full `npm run check:integration` passed all253 tests, zero failures/skips, including actual PostgreSQL module/combined application cases in random disposable schemas on anonymous port55439. Startup regression applies003 idempotently and verifies all three ledger IDs. The combined HTTP test confirms a relationship and current affiliation, verifies exact receipt retry/stale conflict, obtains a qualified path through the installed resolver/engine, and checks unauthorized access. One initial test incorrectly expected400 for an oversized body; corrected to the existing413 behavior, then the entire gate passed. Server/browser production build passed (34 browser modules).

No private live database migration, live provider call, restart, frontend merge, push or deployment occurred. The running OAuth process remains on its earlier commit; on-disk build includes these changes. Root must coordinate restart and acceptance. Manual facts are optional infrastructure; link-first public discovery remains separate upcoming integration. No source-owner parser/auth/storage/graph implementation was edited outside the exact imported facts module.

## Durable public collection checkpoint

Merged exact Nicolas main14dce5d6864168f55ae0c3e9b37b4c77b7ed0922 as6a337d1, preserving zero-based events; build passed. Imported cleared source68214d3/Tavily077d9bc/robots006bb1c as211c0bf/1029085/044f859. No pull or main mutation; no push. Existing Vite may hot-reload changed frontend sources, but the API process was not restarted.

Authenticated GET /api/discovery/capabilities and same-origin POST /api/discovery now compose the frozen source-only DTOs with durable private receipts. Main injects the reviewed Tavily provider from server process TAVILY_API_KEY only; no key file is read by modules and no key is passed to the browser environment. Missing key reports NOT_CONFIGURED. GET performs no provider request; AVAILABLE means a successful observed provider result within60seconds, not merely configuration. Discovery collection still returns no extracted proposals and explicitly warns extraction/persistence are not implemented. Only its safe frozen result is durably retained; raw hits/text/envelopes are not HTTP responses or graph writes.

Migration005_discovery_receipts owns discovery receipts, independent of reserved public-facts004. Session-before-scope transactions claim/finalize requests; full canonical request including original version binds the owner/scope/key. Exact completed retries reauthorize and return the saved result without provider calls, even after an unrelated graph version advance. Altered requests conflict; active duplicates conflict; expired/failed attempts fail closed and require a deliberate new key rather than silently repeating possibly billed work. No locks span network waits. Logout, context/source policy changes and mid-flight graph mutations block finalization. Client disconnect cancels collection.

Selected context terms come only from deliberately selected CONFIRMED public-source identities under public-citation-review-v1 with identity evidence. Private Person fields/Contacts names are not query terms. No public identity evidence means FORBIDDEN. Anchors stay self-asserted, not verified ownership.

New server-only PublicSourceProvisioner reuses session-before-scope transactions and derives stable source IDs from actor/scope/public document URL. It registers only PUBLIC_PROFILE/PUBLIC_ARTICLE with PUBLIC_SOURCE/public-citation-review-v1, no verifiedOwnerIdentity, and returns each committed graph version. Existing enabled same-policy source retries do not increment again; stale new source creation conflicts; revoked sessions fail. Collection does not call provisioning yet; later cleared producer/staging composition must sequence its returned versions and durable stage receipts explicitly. This checkpoint claims no atomic collection-to-staging pipeline.

Focused verification: strict server build and41 offline source/Tavily/HTTP/application/PostgreSQL tests passed, zero skips, using anonymous disposable55439schemas and injected transports. Fixed a test-discovered capability bug: a request conflict no longer clears provider health; observations also expire after60seconds. No secret reads, live searches, privateDB migrations, API restarts, extractor/planner/public-facts enablement or deployment occurred.

## Cleared public-citation staging and identity facade

Imported reviewed public-facts e8bccea plus normalized-text correction2b1bbd as8919895/12a3631. Application now awaits migration004 before005 and constructs PublicFactsService/PgPublicFactsStore with the shared real auth/pool. Server-internal publicFacts.stage and session-aware publicSources.provision are available for the future reviewed producer bridge; no browser stage/envelope upload route exists.

Authenticated GET /api/public-facts/review?scopeId=...&batchId=... returns safe cited proposals/endpoints; same-origin POST /api/public-facts/resolve accepts the strict reviewed explicit NEW_PERSON/LINK_EXISTING request and returns committed identity decision/version/events. Existing16KiB body limit, opaque-session-derived actor/hash, source/version/revision/idempotency checks, no-store and error sanitization apply. Full document text, private payload refs and session hashes stay out of review responses. Resolving identity does not accept public relationships or add search edges; proposals remain pending.

Full combined check:integration passed316 tests, zero failures/skips, using only anonymous scratch55439. This includes startup migrations001–005, durable collection/receipts, actual source provisioning -> private staging -> HTTP review/identity resolution, exact retry/stale guards, private stage route404, and no relationships/edges after explicit new-person identity resolution. Server/browser build passed34modules. No app restart, live searches/provider request, secret reads, privateDB migration, push or deployment. Extractor/planner remain held for independent clearance; relationship acceptance/policy remains its owner's upcoming006 module.

## Cleared planner/extractor to recoverable public staging

Imported exact extractor8b12eed/correctionb4359ac and planner4b31e4f asd2ea7d1/8cc7f69/e13c69d. Replaced the initial source loop with one createDiscoveryPlanner using real extractPublicClaimFragments; existing source/search/document transports and their safety boundaries are preserved. Global planner limits remain4searches/8retainedhits/5documentattempts/30seconds; no per-hop restart of those budgets. Search snippets and denied assertions do not become proposals.

Append-only migration007_discovery_staging (explicitly reserved by root) adds private retained workflow/run fencing and an atomic source-step receipt. Migration005 checksum is unchanged;006 remains public acceptance owner's reserved number. Exact observations/text/retrievedAt/revisions are retained privately after reauthorization. Missing public evidence persists an honest insufficient/source-unavailable receipt without creating sources. Documents with extracted assertions proceed individually through session-aware source provisioning and the authorized producer to PublicFactsService.stage. Each provision/stage uses the preceding committed version; public source metadata never asserts verified owner identity. All independence groups remain conservatively unassessed, rather than claiming independent corroboration.

Source provisioning operation receipts and exact stage requests are saved before continuing. A source/stage commit followed by lost response/checkpoint can be resumed using the original immutable inputs and idempotency keys, without new searches or refetches. No workflow means an expired/failed collection still cannot automatically repeat potentially billed provider calls. Existing retained work can resume under a new fenced run after failure/lease expiry, with live authorization and document/source checks. Newer workers fence old journal writes; independently idempotent source/stage transactions prevent duplicate writes. Locks never span provider waits. The operation is a recoverable sequence of private commits, not one atomic network-to-graph transaction.

DiscoveryResult keeps its original baseGraphVersion and frozen shape. Actual stored proposals supply proposalRefs; REVIEW_REQUIRED is returned only after stage receipts exist and current proposal/document heads/source policies are verified. Raw texts, envelopes, keys and excerpts are not serialized in this response. Old observation/source withdrawal prevents replay of misleading references. Identity resolution and reviewed public relationships remain separate; no identities are silently merged or public search edges created.

Verification: full check:integration358tests passed, zero failures/skips, on anonymous scratch55439 only; server/browser production build passed34modules. New actual application tests cover two-source expansion/staging with graph versions2/4, exact retry without calls, source-commit and stage-response interruptions, disabled source/logout recovery denial, changed policy/document-head rejection, and denied assertions/snippets producing no graph/staged proposals. No privateDB/provider calls, secret-file reads, API restart, scopes/services, push or deployment occurred. Existing API remains the prior live process; this implementation requires observer-reviewed restart/acceptance. Public006 acceptance and actual real multi-hop route remain unfulfilled gates.
