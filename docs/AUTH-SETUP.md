...
## Separately consented Google Contacts

Login itself still supplies **no Contacts access**. The additive server module and its setup are now described in [CONTACTS-AUTH-SETUP.md](CONTACTS-AUTH-SETUP.md). It uses separate consent/callback routes, actor/session-bound one-time transactions, a token encryption key and `ContactsStore`. It requests `contacts.readonly` only after the user explicitly chooses Connect Contacts. Its presence does not imply that routes, storage migration, Google API configuration or live retrieval are already deployed.
